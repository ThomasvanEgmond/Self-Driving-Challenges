...
def driving_algorithm(parsed_data):
    ...
    if not waiting_on_green_light: 
        # print("NOT WAITING ON GREEN LIGHT")
        des_speed = sign_speed
    else: 
...
def steering_logic(): # run in thread
    while True:
        ...
        if bool(left_camera_steering_percentage) and not bool(right_camera_steering_percentage):
            ...
            continue
            # esp32_data["steering_degrees"] = 180
...
def rev_counter_start():
    global rev_parent_pipe
    rev_parent_pipe, rev_child_pipe = Pipe()
    rev_process = Process(target=Revolutions().get_revolutions, args=(rev_child_pipe,), daemon=True)
    rev_process.start()
